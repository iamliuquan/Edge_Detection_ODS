...
def edges_eval_img(im, gt, out="", thrs=99, max_dist=0.0075, thin=True, need_v=False, workers=1):
    ...
    cls_info = loadmat(gt)['GTcls']

    seg_mask_arr = cls_info[0, 0][1]
    seg_mask_arr[seg_mask_arr > 0] = 1

    # process edge
    seg_mask_arr = cls_info[0, 0][1]
    object_label = cls_info[0, 0][2][:, 0]

    canvas = np.zeros_like(cls_info[0, 0][1])
    for label in object_label:
        contour_arr = cls_info[0, 0][0][label - 1][0].todense()
        contour_arr = np.array(contour_arr)
        canvas += contour_arr
    gt = canvas
...
def edges_eval_dir(res_dir, gt_dir, cleanup=0, thrs=99, max_dist=0.0075, thin=True, workers=1):
    ...
    """
    eval_dir = '/media/liuq23/ssd2/edge_detection/ODS/edge_eval_python/examples/zero_crossing_siemenstar_t15-eval'
    ...
    ids = [os.path.split(file)[-1] for file in glob.glob(os.path.join(gt_dir, "*.mat"))[:1135]]  # 1135
